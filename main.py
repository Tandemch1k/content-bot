...
async def send_for_moderation(bot: Bot, post: dict):
    ...
    tg_text = post["telegram_text"][:1020]
    image_url = post.get("image_url")

    # Отправляем фото + текст вместе, потом кнопки модерации
    if image_url:
        try:
            await bot.send_photo(chat_id=ADMIN_ID, photo=image_url, caption=f"👁 Предпросмотр поста")
        except Exception as e:
            logger.warning(f"Preview photo failed: {e}")

    # Текст поста + кнопки одобрения
    preview = f"{type_emoji} Новый пост | ID: {post['id']}\n\n{tg_text}"
...
async def generate_and_queue(bot: Bot):
    ...
    try:
        ...
        posts = generator.generate_batch(articles, count=3)
...
async def _publish_post(query, post: dict):
    ...
    # Кнопки со ссылками на платформы
    channel_buttons = InlineKeyboardMarkup([[
        InlineKeyboardButton("📸 Instagram", url="https://www.instagram.com/juliia_mart?igsh=MzR0anMycTg4M2V2"),
        InlineKeyboardButton("🎥 YouTube", url="https://m.youtube.com/@top_family_dxb"),
        InlineKeyboardButton("✈️ Telegram", url="https://t.me/top_family_manager"),
    ]])

    # Публикация в Telegram канал — фото + текст всегда вместе
    try:
        bot = query.get_bot()
        text = post["telegram_text"][:1020]
            ...
            for parse_mode in ["HTML", None]:
                try:
                    ...
                    if parse_mode is None:
                        raise e
                    logger.warning(f"Parse failed, trying plain: {e}")
        else:
            for parse_mode in ["HTML", None]:
                try:
                    kwargs = {
