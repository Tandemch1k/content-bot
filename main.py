...
async def send_for_moderation(bot: Bot, post: dict):
    ...
    if image_url:
        try:
            await bot.send_photo(chat_id=ADMIN_ID, photo=image_url)
        except Exception as e:
            logger.warning(f"Failed to send photo: {e}")

    # Отправляем текст — сначала с Markdown, если ошибка — без
    for parse_mode in ["Markdown", None]:
        try:
            ...
            if parse_mode is None:
                raise e
            logger.warning(f"Markdown parse error in moderation, trying plain: {e}")
