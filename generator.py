...
def _generate_photo_query(text: str, post_type: str) -> str:
    """Генерирует поисковый запрос для Pexels на основе текста поста."""
    prompt = f"""
На основе этого текста поста о недвижимости Dubai сгенерируй короткий поисковый запрос (3-5 слов на английском) для поиска подходящего фото на Pexels.

Текст поста:
{text[:500]}

Правила:
- Только 3-5 слов на английском
- Конкретный запрос (например: "Dubai Marina luxury apartment", "Palm Jumeirah aerial view", "modern interior living room")
- Соответствует главной теме поста
- Только запрос, никакого другого текста
...
Запрос:"""
    ...
    try:
        ...
        logger.info(f"Photo query generated: {query}")
        return query
    except Exception as e:
        logger.warning(f"Photo query generation failed: {e}")
        # Fallback по типу
        defaults = {
            "news": "Dubai skyline luxury",
            "tip": "Dubai apartment interior",
            "insight": "Dubai aerial cityscape",
        }
        return defaults.get(post_type, "Dubai real estate")
